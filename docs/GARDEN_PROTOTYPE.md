...
The review toolbar's **View: Auto / View: Swapped** button inverts the landscape composition for the current viewport. It switches the lens, shoreline width, tree placement, and scene settings together, while the invitation text and controls continue fitting the actual screen. Toggle it again to restore automatic selection; it also works while motion is paused and retains the current chapter.
