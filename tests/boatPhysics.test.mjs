...
test('the lighter duck reacts more than the tug and its small hull allows close passes', () => {
  const tug = body(1, 'tugboat'), duck = body(3, 'duck');
  Object.assign(tug, { x: -.20, vx: .5 }); Object.assign(duck, { x: .20 });
  resolveBoatContact(tug, duck);
  assert.ok(Math.abs(duck.vx) > Math.abs(tug.vx - .5) * 4);
  const a = body(3, 'duck'), b = body(6, 'duck'); b.x = .45;
  assert.equal(boatContact(a, b), null);
});
...
test('overlapping launches separate without an explosive impulse, including paused launches', () => {
  ...
  for (let id = 1; id <= 12; id++) {
    world.launch({ id, kind: id % 3 === 0 ? 'duck' : 'paper', lateral: 0, z: 8 }, MOBILE_RIVER_WIDTH);
...
test('fixed timesteps give matching collision paths at 30 and 120 fps', () => {
  const run = fps => {
    const world = new BoatSimulation(river, MOBILE_RIVER_WIDTH);
    for (let id = 1; id <= 4; id++) world.launch({ id, kind: id === 3 ? 'duck' : 'paper', lateral: 0, z: 8 }, MOBILE_RIVER_WIDTH);
